...
export default function subscribeFirebase(localBinding, bindOptions, options) {
  ...
  if (type === "Array") {
      ...
      if (initialValueReceived) {
        if (populate) {
          const ref = firebase
            .app(name)
            .database()
            .ref(populate(snapshot.key))
          ref.once(
            "value",
            populatedSnapshot => {
              dispatchChildAdded(store, localBinding)(
                snapshot.key,
                populatedSnapshot.val(),
                previousChildKey
              )
              populated[snapshot.key] = {
                ...
                listener: ref.on(
                  "value",
                  populatedSnapshot => {
                    dispatchChildChanged(store, localBinding)(
                      snapshot.key,
                      populatedSnapshot.val()
                    )
                  },
    ...
    const onceValue = snapshot => {
      if (populate) {
        const populateRefs = {}
        Promise.all(Object.keys(snapshot.val() || {}).map(populateChild))
          .then(resolved => {
            const result = []
          ...
          .then(populatedResult => {
            dispatchArrayUpdated(store, localBinding)(
              snapshot.key,
              populatedResult
            )
    ...
    listeners.child_changed = query.on(
      "child_changed",
      onChildChanged,
      cancelCallback
    )
    listeners.child_moved = query.on(
      "child_moved",
      dispatchChildMoved(store, localBinding),
      cancelCallback
    )
    listeners.child_removed = query.on(
      "child_removed",
      onChildRemoved,
      cancelCallback
    )
